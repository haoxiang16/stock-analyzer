      ...
      <div v-else>
        <div class="relative overflow-x-auto -mx-4 sm:-mx-6 px-4 sm:px-0 sm:mx-0">
          <table class="table w-full min-w-[800px] lg:min-w-0 shadow-md rounded-lg overflow-hidden">
            ...
            <tbody class="text-sm sm:text-base divide-y divide-slate-700">
              <tr v-for="stock in paginatedStocks" :key="stock.companyCode" 
                  class="hover:bg-slate-800/70 transition-colors duration-150
                         flex flex-col sm:table-row py-4 sm:py-0 even:bg-slate-800/30 bg-slate-900">
                <td data-label="股票代號" class="font-bold py-2 sm:py-6 text-emerald-400 text-base sm:text-xl
                                               flex flex-col sm:table-cell sm:border-b sm:border-slate-700
                                               relative px-4">{{ stock.companyCode }}</td>
                <td data-label="公司名稱" class="font-medium text-slate-300 text-sm sm:text-lg
                                            flex flex-col sm:table-cell sm:border-b sm:border-slate-700
                                            py-2 sm:py-4 relative px-4">{{ stock.companyName }}</td>        
                <td data-label="財務數據" class="flex flex-col sm:table-cell sm:border-b sm:border-slate-700 py-2 sm:py-4 relative px-4">
                    ...
                    <table class="w-full shadow-sm rounded-lg overflow-hidden">
                      <thead>
                        <tr class="bg-slate-800 border-b border-slate-700">
                          <th class="px-3 py-2.5 sm:px-4 sm:py-3.5 text-center font-bold text-xs sm:text-sm text-slate-300">年度</th>
                          <th class="px-3 py-2.5 sm:px-4 sm:py-3.5 text-center font-bold text-xs sm:text-sm text-emerald-400">EPS</th>
                          <th class="px-3 py-2.5 sm:px-4 sm:py-3.5 text-center font-bold text-xs sm:text-sm text-amber-400">營業利益率</th>
                          <th class="px-3 py-2.5 sm:px-4 sm:py-3.5 text-center font-bold text-xs sm:text-sm text-blue-400">毛利率</th>
                          <th class="px-3 py-2.5 sm:px-4 sm:py-3.5 text-center font-bold text-xs sm:text-sm text-purple-400">淨利率</th>
                        </tr>
                      </thead>
                      <tbody class="divide-y divide-slate-700">
                        <tr v-for="(financial, index) in stock.yearlyFinancials" :key="financial.year" 
                            :class="`text-xs sm:text-sm ${index % 2 === 0 ? 'bg-slate-800/80' : 'bg-slate-800/40'} hover:bg-slate-700/70 transition-colors`">
                          <td class="px-3 py-2.5 sm:px-4 sm:py-3.5 text-center whitespace-nowrap font-semibold text-slate-300">
                            {{ financial.year }} 年
                          </td>
                          <td class="px-3 py-2.5 sm:px-4 sm:py-3.5 text-center whitespace-nowrap font-semibold text-emerald-300">
                ...
                <td data-label="操作" class="flex flex-col sm:table-cell sm:border-b sm:border-slate-700 py-2 sm:py-4 relative px-4">
                  <div class="flex flex-col items-center gap-3 px-2 sm:px-4">
                    <button
                      @click="handleAnalyze(stock)"
                      class="btn w-full sm:w-48 text-sm sm:text-base font-medium h-auto bg-slate-700 hover:bg-slate-600 text-white border border-slate-600 shadow-md hover:shadow-lg transition-all duration-300 py-2 sm:py-3 rounded-lg"
                    >
                      <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 sm:h-5 sm:w-5 mr-2 text-emerald-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
                      </svg>
                      分析報告
                    </button>
                    <button
                      ...
                      class="btn w-full sm:w-48 text-sm sm:text-base font-medium h-auto bg-slate-800 hover:bg-slate-700 text-white border border-slate-600 shadow-md hover:shadow-lg transition-all duration-300 py-2 sm:py-3 rounded-lg"
                    >
